import { NextResponse } from "next/server"
import { createHash, randomBytes } from "node:crypto"
import nodemailer from "nodemailer"
import { z } from "zod"
import { findPendingVerificationByEmail, upsertPendingVerification } from "@/lib/email-verification-db"
import { findAccountByEmail } from "@/lib/accounts-db"
import { checkRateLimit, getClientIp, rateLimitResponse } from "@/lib/rate-limit"

export const runtime = "nodejs"

const RESEND_RATE_LIMIT = { name: "resend-verification", maxRequests: 3, windowSeconds: 60 * 60 }
const VERIFY_EXPIRY_SECONDS = 60 * 60 * 24 // 24 heures

const Schema = z.object({ email: z.string().trim().email() })

function hashToken(token: string): string {
  return createHash("sha256").update(token).digest("hex")
}

export async function POST(req: Request) {
  try {
    const ip = getClientIp(req)
    const rl = checkRateLimit(RESEND_RATE_LIMIT, ip)
    if (!rl.allowed) return rateLimitResponse(rl.retryAfterSeconds)

    const { email } = Schema.parse(await req.json())
    const normalizedEmail = email.toLowerCase()

    // Réponse toujours identique pour éviter l'énumération
    const alreadyVerified = await findAccountByEmail(normalizedEmail)
    if (alreadyVerified) return NextResponse.json({ ok: true })

    const pending = await findPendingVerificationByEmail(normalizedEmail)
    if (!pending) return NextResponse.json({ ok: true })

    // Générer un nouveau token et mettre à jour l'entrée
    const token = randomBytes(32).toString("hex")
    const tokenHash = hashToken(token)
    const expiresAt = new Date(Date.now() + VERIFY_EXPIRY_SECONDS * 1000)

    await upsertPendingVerification({
      email: normalizedEmail,
      name: pending.name,
      passwordHash: pending.passwordHash,
      tokenHash,
      expiresAt,
      postalCode: pending.postalCode,
      city: pending.city,
    })

    const smtpHost = process.env.SMTP_HOST?.trim()
    const smtpPort = Number(process.env.SMTP_PORT || "465")
    const smtpUser = process.env.SMTP_USER?.trim()
    const smtpPass = (process.env.SMTP_PASS || "").replace(/\s+/g, "").trim()

    if (!smtpHost || !smtpUser || !smtpPass || Number.isNaN(smtpPort)) {
      console.error("resend-verification: SMTP non configuré")
      return NextResponse.json({ ok: false, error: "Service email non disponible." }, { status: 500 })
    }

    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_SITE_URL
    if (!baseUrl) {
      return NextResponse.json({ ok: false, error: "Configuration serveur incomplète." }, { status: 500 })
    }

    const verifyUrl = `${baseUrl}/verifier-email?token=${token}`

    const transporter = nodemailer.createTransport({
      host: smtpHost,
      port: smtpPort,
      secure: smtpPort === 465,
      auth: { user: smtpUser, pass: smtpPass },
    })

    const html = buildVerificationEmail(pending.name, verifyUrl)

    await transporter.sendMail({
      from: `"PitStop" <${smtpUser}>`,
      to: normalizedEmail,
      subject: "Confirmez votre adresse email : PitStop",
      text: `Bonjour ${pending.name},\n\nCliquez sur ce lien pour confirmer votre adresse email (valable 24 h) :\n${verifyUrl}\n\nSi vous n'avez pas créé de compte PitStop, ignorez cet email.`,
      html,
    })

    return NextResponse.json({ ok: true })
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ ok: false, error: "Email invalide." }, { status: 400 })
    }
    console.error("resend-verification error:", error)
    return NextResponse.json({ ok: false, error: "Erreur serveur." }, { status: 500 })
  }
}

export function buildVerificationEmail(name: string, verifyUrl: string): string {
  return `
    <div style="font-family:Arial,Helvetica,sans-serif;background:#f8fafc;padding:24px;">
      <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;">
        <div style="padding:18px 20px;background:linear-gradient(90deg,#0f172a,#1e3a8a);color:#ffffff;">
          <div style="font-size:18px;font-weight:700;">PitStop : Confirmez votre adresse email</div>
        </div>
        <div style="padding:24px 20px;">
          <p style="margin:0 0 16px;color:#334155;font-size:15px;">Bonjour ${name},</p>
          <p style="margin:0 0 16px;color:#334155;font-size:14px;">
            Merci de vous être inscrit sur PitStop ! Pour activer votre compte,
            veuillez confirmer votre adresse email en cliquant sur le bouton ci-dessous.
          </p>
          <p style="margin:0 0 24px;">
            <a href="${verifyUrl}"
               style="display:inline-block;background:#1e3a8a;color:#ffffff;text-decoration:none;padding:12px 28px;border-radius:8px;font-size:15px;font-weight:600;">
              Confirmer mon adresse email
            </a>
          </p>
          <p style="margin:0 0 8px;color:#64748b;font-size:13px;">
            Ce lien est valable pendant <strong>24 heures</strong>.
            Si vous n'avez pas créé de compte PitStop, ignorez cet email.
          </p>
        </div>
      </div>
    </div>
  `
}
